...
- **Píldora de escribir (dock):** una fila — campo · ubicación · «+» · Voz/Enviar — con radio fijo 28, alto 56 con una línea y 128 como máximo (cuatro líneas completas: `ALTO_MAX_CAMPO` = 4 × 24 + 20 de aire = 116). El aire del campo es un **borde transparente** de 10 px arriba y abajo, no padding: un textarea con scroll pinta texto dentro de su padding y la primera línea quedaba pegada al canto de la píldora. Fondo `var(--surface-1)`; sobre el aura, `var(--home-dock-bg)`. Campo a `1rem` (por debajo de 16 px iOS hace zoom al enfocar), placeholder en una línea y en `var(--text-dim)` (`.dock-input`). Voz y Enviar son el **mismo** círculo de 44 con aro `--teal-text`: escribir la primera letra no mueve nada. Los botones son solo icono y llevan `aria-label`. El alto y el overflow los gobierna `ajustarAltoCampo` (App.jsx), no el `style`. El dock del Mapa Vivo (`MapView.jsx`) aún no adopta esta receta.
