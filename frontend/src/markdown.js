  ...
  while (i < lines.length) {
    ...
    if (/^[-*]?\s*(✅|⚠)️?\s*\S/.test(t)) {
      ...
      while (i < lines.length) {
        ...
        if (!mm) break
        const ok = mm[1] === '✅'
        rows.push(`<div class="enc-fila" style="display:flex;gap:8px;align-items:flex-start;padding:5px 0;line-height:1.45;"><span style="flex-shrink:0;">${ok ? '✅' : '⚠️'}</span><span class="${ok ? 'enc-ok' : 'enc-warn'}">${inline(mm[2])}</span></div>`)
        i++
      }
