...
- **Píldora de escribir (dock):** dos formas sobre el mismo DOM, con radio fijo 28. **Una fila** — campo · ubicación · «+» · Voz/Enviar, alto 56 — mientras el campo está vacío o el texto cabe en una línea. **Amplia** en cuanto el texto no cabe: el campo ocupa todo el ancho y los botones bajan a una segunda fila (ubicación y «+» a la izquierda, Voz/Enviar a la derecha); con los botones al lado, en un teléfono el texto se quedaba en una columna de ~180 px. El cambio lo decide `noCabeEnUnaLinea` (App.jsx) midiendo el **texto** en un canvas, nunca el layout actual (oscilaría), y lo marca con `data-amplio` en `.dock-pill`; lo que cambia con la forma vive en `index.css`, no en el `style`. El campo crece hasta cuatro líneas completas (`ALTO_MAX_CAMPO` = 4 × 24 + 20 de aire = 116). El aire del campo es un **borde transparente** de 10 px arriba y abajo, no padding: un textarea con scroll pinta texto dentro de su padding y la primera línea quedaba pegada al canto de la píldora. Fondo `var(--surface-1)`; sobre el aura, `var(--home-dock-bg)`. Campo a `1rem` (por debajo de 16 px iOS hace zoom al enfocar), placeholder en una línea y en `var(--text-dim)` (`.dock-input`). Voz y Enviar son el **mismo** círculo de 44 con aro `--teal-text`: escribir la primera letra no mueve nada. Los botones son solo icono y llevan `aria-label`. El alto y el overflow los gobierna `ajustarAltoCampo` (App.jsx), no el `style`. El dock del Mapa Vivo (`MapView.jsx`) aún no adopta esta receta.
